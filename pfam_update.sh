...
#JOB_ID=$(sbatch -p hh -t 2-0 -n ${NCORES} -N 1 --parsable -o "${LOG_DIR}/pfam_hhblits.log" -d "afterok:$JOB_ID" --array=1-11 ./pfam_hhblits.sh)

CS_JOB_ID=$(sbatch -p hh -t 2-0 -c ${NCORES} -N 1 --parsable -o "${LOG_DIR}/pfam_cstranslate.log" -d "afterok:$HH_JOB_ID" --kill-on-invalid-dep=yes ./pfam_cstranslate.sh)
HM_JOB_ID=$(sbatch -p hh --exclude=hh003 -t 2-0 -n ${NCORES} -N 1 --parsable -o "${LOG_DIR}/pfam_hhmake.log" -d "afterok:$HH_JOB_ID" --kill-on-invalid-dep=yes ./pfam_hhmake.sh)
